package com.example.muhtamimnahid.doctorapp.helper;
...
class Activity_GPUImageFilterTools {

    static void showDialog(final Context context,
                                  final OnGpuImageFilterChosenListener listener) {
        final FilterList filters = new FilterList();
        filters.addFilter(context.getString(R.string.filter_1), FilterType.CONTRAST);
        filters.addFilter(context.getString(R.string.filter_2), FilterType.HUE);
        filters.addFilter(context.getString(R.string.filter_3), FilterType.BRIGHTNESS);
        filters.addFilter(context.getString(R.string.filter_4), FilterType.SHARPEN);
        filters.addFilter(context.getString(R.string.filter_5), FilterType.SATURATION);
        filters.addFilter(context.getString(R.string.filter_6), FilterType.EXPOSURE);
        filters.addFilter(context.getString(R.string.filter_7), FilterType.HIGHLIGHT_SHADOW);
        filters.addFilter(context.getString(R.string.filter_8), FilterType.WHITE_BALANCE);
        filters.addFilter(context.getString(R.string.filter_9), FilterType.COLOR_BALANCE);
    ...
    private enum FilterType {
        CONTRAST, SHARPEN, BRIGHTNESS, HUE,
        SATURATION, EXPOSURE, HIGHLIGHT_SHADOW, WHITE_BALANCE,
        COLOR_BALANCE, LEVELS_FILTER_MIN
    }

    private static class FilterList {
        final List<String> names = new LinkedList<>();
        final List<FilterType> filters = new LinkedList<>();

        void addFilter(final String name, final FilterType filter) {
            names.add(name);
            filters.add(filter);
        }
    }

    static class FilterAdjuster {
        private final Adjuster<? extends GPUImageFilter> adjuster;

        FilterAdjuster(final GPUImageFilter filter) {
        ...
        void adjust(final int percentage) {
            if (adjuster != null) {
        ...
        private abstract class Adjuster<T extends GPUImageFilter> {
            ...
            float range(final int percentage, final float start, final float end) {
